    ...
    for fname in fnames:
        ...
        for annotation_segment_index in range(len(segment_annotation)):
            if measurements_info["TYPE"] == "trajectories":
                ...
                number_of_targets = len(target_info)
                for target_index in range(number_of_targets):
                    target_index_corrected = target_index + 1
                    target_labels = np.array([target_info[target_index_corrected][i]["label"] for i in range(len(target_info[target_index_corrected]))])
                    current_annotation_labels = segment_annotation["label"][annotation_segment_index:annotation_segment_index+len(target_labels)].to_numpy()
                    if np.array_equal(target_labels,current_annotation_labels):
                        token_number += 1
                        for tmp_segment_idx in range(len(target_labels)):
                            target_channel = target_info[target_index_corrected][tmp_segment_idx]["channel"]
                            target_dimension = target_info[target_index_corrected][tmp_segment_idx]["dimension"]
                            target_parameter = target_info[target_index_corrected][tmp_segment_idx]["parameter"]
                            signal, _ = sp.get_signal(data=files[fname].ema,
                                                channel_dict=channel_dict,
                                                target_channel=target_channel,
                                                target_dimension=target_dimension,
                            ...
                            for measurements_index in range(len(measurements_parts)):
                                ...
                                if measurements_info["measurements"][measurements_index]["target"] == target_labels[tmp_segment_idx]:
                                    is_measured = True
                                    colname = measurements_info["measurements"][measurements_index]["target"] + "_" +  measurements_info["measurements"][measurements_index]["landmark"] + "_" + measurements_info["measurements"][measurements_index]["parameter"]
                                    tmp_tmin = segment_annotation["tmin"][annotation_segment_index+tmp_segment_idx]
                                    tmp_tmax = segment_annotation["tmax"][annotation_segment_index+tmp_segment_idx]
                                    mid = (tmp_tmin + tmp_tmax)/2
                                    landmark_annotation = annotation[annotation["tierName"] == target_info[target_index_corrected][tmp_segment_idx]["landmark tier"]].reset_index(drop=True)
                                    
                                    measurement_landmark = measurements_info["measurements"][measurements_index]["landmark"]
                                    ...
                                    if measurements_info["measurements"][measurements_index]["parameter"] == "time":
                                        values[colname] = target_landmark_time
                                    elif measurements_info["measurements"][measurements_index]["parameter"] == "position":
                                        time_idx = np.abs(target_landmark_time - files[fname].ema.time.values).argmin()
                                        values[colname] = signal[time_idx]
                                    elif measurements_info["measurements"][measurements_index]["parameter"] == "velocity":
                                        time_idx = np.abs(target_landmark_time - files[fname].ema.time.values).argmin()
